...
from net import FallDetectionModel  # 假设您有一个 net.py 文件定义了模型
...
def custom_collate_fn(batch):
    angles, labels = zip(*batch)
    lengths = torch.tensor([len(x) for x in angles], dtype=torch.int64)
    padded_angles = pad_sequence([a.clone().detach() for a in angles], batch_first=True, padding_value=0)
    labels = torch.tensor(labels, dtype=torch.long)
    return padded_angles, labels, lengths

def main():
    # Paths to dataset
    train_dir = "data/train/videos"
    train_json_dir = "data/train/keypoints"
    test_video_dir = "data/test/videos"
    test_json_dir = "data/test/keypoints"

    # Training Dataset and DataLoader
    train_dataset = FallDetectionVideoDataset(video_dir=train_dir, json_dir=train_json_dir)
    ...
    test_dataset = FallDetectionVideoDataset(video_dir=test_video_dir, json_dir=test_json_dir)
